...
def contact_solve(model, mech, qpos, qvel=None, ctrl=None):
    """Run the model's own contact law on MuJoCo mechanics at one state.

    Dispatches on ``contact_solver``.  The constraint branch returns its solver
    dict directly.  The compliant branch is a penalty force rather than a solve,
    so its Hunt--Crossley quantities are packed into the same key names the rest
    of this module consumes: ``contact_acceleration`` from the generalized force
    through MuJoCo's mass matrix, and ``normal_force`` from the per-contact
    Hunt--Crossley magnitude.
    """
    ...
    with th.no_grad():
        if model.contact_solver == "compliant":
            g, gdot, v_t, lam, f_t, J_n, J_t = model._contact_parts(pos, qd)
            force = model._contact_force_gen(pos, qd)
            acc = th.linalg.solve(Mt, force.unsqueeze(-1)).squeeze(-1)
            k, c, mu = th.nn.functional.softplus(model._contact_raw)
            out = {
                "contact_acceleration": acc,
                ...
                "normal_force": lam,
                "tangent_force": f_t,
                # The penalty law's own engagement measure, so ``gate`` keeps a
                # meaning in the shared summary: the smoothed penetration
                # normalized by its value at zero gap.
                "gate": (
                    th.nn.functional.softplus(-g / model._contact_gap_width)
                    / float(np.log(2.0))
                ),
                # A penalty law solves nothing, so there is no residual to
                # report and the cone is satisfied by construction of f_t.
                "solver_residual": th.zeros(pos.shape[0], dtype=dtype),
                "cone_violation": th.zeros(pos.shape[0], dtype=dtype),
                "mu": mu,
                "e": k,
                "beta": c,
                "regularization": th.zeros((), dtype=dtype),
            }
        else:
