...
# arm name -> constructor differences.  Everything else is shared: K = 6,
# constraint solver, hidden (128, 128), 12 ADMM iterations, regularizer 1e-2.
ARMS = {
    "learned_constraint": dict(contact_geometry="learned", contact_compliance=None),
    "kinematic_constraint": dict(contact_geometry="kinematic", contact_compliance=None),
    "kinematic_compliance": dict(contact_geometry="kinematic", contact_compliance=True),
    "kinematic_compliance_frozen": dict(
        contact_geometry="kinematic", contact_compliance=True
    ),
}
...
def build(arm, obs_dim, act_dim, contact_dt):
    cfg = ARMS[arm]
...
def main():
    ...
    for name in sorted(os.listdir(args.models_dir)):
        ...
        if arm not in ARMS:
            print(f"  skip {tag}: unknown arm", flush=True)
            continue
        model = build(arm, obs_dim, act_dim, max_step)
