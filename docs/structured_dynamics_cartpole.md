...
1. **Flow-consistent, duration-balanced fitting.** Every 2–30 ms replay interval is fully integrated at 2 ms internal resolution. Endpoint losses are weighted by $(2\text{ ms}/\Delta t)^2$, so the sparse 30 ms tail no longer supplies roughly 95% of the drift-error leverage. The first 5,000 dynamics fits use $H=1$; thereafter the model is fit every second critic update, with local $H=1$ supervision normally and the full $H=4$ rollout every fourth fit. Long transitions and open-loop training remain present without paying for a worst-case 60-step BPTT graph every step.
2. **Accepted target dynamics.** The optimizer mutates a live model, but the critic reads a frozen copy. Cheap loss/gradient/parameter checks run after every fit. Every tenth fit, a candidate is published only after finite recorded-duration and nominal-duration flows on an independent replay batch and a flow error below the configured threshold. The first accepted model is copied exactly; later accepted versions enter with the cadence-correct EMA $\tau_K=1-(1-\tau)^K$. A non-finite candidate is rolled back to the last accepted model and its Adam state is cleared.
3. **Staged takeover and diagnostics.** The cartpole run requires 10,000 dynamics fits, at least one accepted publication, and a ready scalar value head before learned quadrature replaces the model-free warmup target. Healthy target evaluation performs one aggregate finite check; only a failure invokes component-by-component and internal-step diagnostics. Logs include flow error, publication/rejection/rollback counts, gradient norm, mass eigenvalue/condition diagnostics, rolled-state magnitude, and value-increment magnitude.
