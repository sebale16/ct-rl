...
**The first-order generator is valid only for small $\lVert b\,\Delta t\rVert$.** The term $(b\cdot\nabla V)\,\Delta t$ approximates $V(x') - V(x)$ well only when the effective step is small relative to the observation scale ($\sim O(1)$). The table compares two ways to turn the oracle drift into an estimate of the value change: the first-order autograd term $g_1 = \nabla V\cdot(b\,\Delta t)$, and a two-point finite difference of $V$ at the current state and the model-predicted endpoint, $\text{FD} = V(x + b\,\Delta t) - V(x)$. Both are correlated with the true $\Delta V = V(x') - V(x)$ (measured on a 5k-step-trained MLP critic, a noise-free deterministic-policy $V$, and the MuJoCo oracle drift, $N=512$ transitions per row):

| $\Delta t$ | $\lVert b\,\Delta t\rVert$ | $\lVert\Delta x - b\,\Delta t\rVert/\lVert\Delta x\rVert$ | corr($g_1$, $\Delta V$) | corr(FD-forward, $\Delta V$) |
|---|---|---|---|---|
| 0.001 (below floor) | $\approx 0.55$ | 0.03 | 0.78 | **1.00** |
| 0.002 (physics floor) | $\approx 1.1$ | 0.05 | 0.51 | **1.00** |
...
**Second-order, and why it does not rescue the floor.** Adding $\tfrac12 (b\,\Delta t)^\top \nabla^2 V (b\,\Delta t)$ from the *autograd* Hessian leaves the correlation essentially unchanged at the floor (the $g_1 \to g_2$ move is flat; the real-MLP row below is $0.24 \to 0.24$). The finite difference of §6's first table gets the same curvature from two clean $V$-evaluations instead, which is why it lifts the correlation where the autograd Hessian does not. A decomposition (quadratic $V$, where the Taylor expansion is *exact*, against the real MLP critic) shows two distinct residuals, each dominating in a different regime. The two estimates compared against the true $\Delta V = V(x') - V(x)$ are
$$
...
- **At larger $\Delta t$, the displacement mismatch becomes a ceiling.** It grows with $\Delta t$ (5% at the floor $\to$ 24% at $\Delta t = 0.01$) and enters at *first order* in $\nabla V$ (as $\nabla V\cdot(\Delta x - b\,\Delta t)$), so a $V$-side term — gradient or Hessian — leaves it in place (the exact-$V$ Hessian caps at 0.976, short of 1.0, at $\Delta t = 0.01$). Closing it requires a more accurate *dynamics* displacement. The FD-forward column reaches exactly this ceiling (corr $\approx 1 - \text{mismatch}$), which isolates the residual as the displacement accuracy alone. With the oracle the mismatch is MuJoCo's own semi-implicit integration bias (the realized $\Delta x$ is damped relative to the continuous $b\,\Delta t$), shrinking as $\Delta t\to 0$ and closable by matching the integrator; with a *learned* drift the mismatch is model error, which does not shrink with $\Delta t$ and compounds over a multi-step rollout.

**Conclusion.** The *autograd* first-order generator's valid regime ($\lVert b\,\Delta t\rVert \ll 1$) sits *below* cheetah's physics resolution, so that target is biased across the legitimate control range; it suits genuinely low-drift / fine-timescale systems (e.g. the trading environment). The floor $\Delta t = 0.002$ (autograd first-order corr $\approx 0.5$, finite-difference-over-states corr $\approx 1.0$) was the borderline worth testing, and it was tested at 12 seeds (§7): the generator improves per-update target variance, while model-free is equal on final policy and ahead at peak, since the value-iteration's contraction property governs the outcome. Reading $V$ at the model-predicted endpoint (the finite difference of the first table) removes the $\nabla V$-quality limit and reaches the displacement-mismatch ceiling, which moves the remaining bottleneck onto the dynamics model's accuracy: the oracle drift keeps the target correlated ($\gtrsim 0.9$) out to an 8-step rollout, a learned drift does so on smooth systems (cartpole, corr rising with horizon) but compounds error on contact-rich cheetah (rollout error past 100% by 4 steps, target correlation down to $\approx 0.3$).
