...
- [ ] **M0 — plumbing and oracle ($\sigma=0$, known/MuJoCo).** Add the generator-target path and obtain $b$ from `dmc.dynamics_terms`. Validation: on sampled transitions, the model-based quantity $b\cdot\nabla V-\beta_{\text{phys}}V$ must match the finite-difference `fraction` to within $\mathcal{O}(u)$. Confirm that a `cheetah-run` run matches or exceeds the baseline.
- [ ] **M1 — learned drift (PHAST).** Train `PortHamiltonianModel` from the replay buffer (pretraining followed by online fine-tuning) and switch `dynamics_source="phast"`. Validate one-step prediction accuracy and that the learned-drift $q$ approximates the oracle $q$.
