...
Each training output directory must be new, unless the run is being continued
with `--resume` (see below). The commands below use seed 0; repeat the trained
arms with seeds 1–5 for the six-seed comparison.
...
The `step_*.pth` files are weights only. Evaluation never updates weights. Use
`--splits validation` for development; reserve the interpolation and
extrapolation tests until the protocol is fixed.

## Chunked training on a queue with a wall time

`--max-seconds` writes a full resumable checkpoint under `<output>/checkpoint`
when the budget or a SIGTERM arrives, then stops cleanly; `--resume` continues
that cell at its exact timestep. The checkpoint is `common.checkpoint`'s
(weights, replay buffer, optimizers, counters, entropy temperature, RNG) plus
this experiment's own sampling state: the configuration and reset streams and
the episode counter live on the environment, so without them a resumed chunk
would replay the same plant sequence from the top and skew training toward its
first configurations. A resumed chunk is refused unless the arm, seed,
configuration, manifest, budget and algorithm settings match the paused run,
and `run.json` counts the chunks. `final_model.pth` appears only when a cell
reaches its full budget, so it is a safe completion marker for a chain.

One artifact of chunking: the agent constructor resets the environment before
the checkpoint's sampling state is restored, so `episodes.jsonl` gains one
duplicate low-index row per resume. Read the file as the last row per episode
index; the transitions actually trained on come from the resets after that.

[The chain script](../benchmarks/experiment3_train.slurm) runs both trained arms
with six seeds each as twelve cells on one LS6 `development` node, 100-minute
chunks under the 2 h wall, resubmitting itself until every cell is finished.

```sh
sbatch benchmarks/experiment3_train.slurm
```
