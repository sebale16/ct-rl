...
The construction is system-agnostic: every system fact enters through the DOF layout (§2.6) and the ports. Two companion notes instantiate it end to end — [**Cartpole**](structured_dynamics_cartpole.md), the minimal smooth example (2 DOF, one actuator, no contact) where every learned object has a closed form and which serves as the cheap validation rung, and [**Cheetah**](structured_dynamics_cheetah.md), the contact-driven benchmark (9 DOF, 6 actuators) whose audit findings forced the cyclic-coordinate handling and the contact port of §2.6–2.7.
:::
...
\dot p = -\frac{\partial H}{\partial q} - D\,\dot q + G_a\,a .
$$

Two learned objects enter here beyond $M$ and $V$: the **damping** $D$ and the **actuator port** $G_a$ (action to generalized force). The damping is a learned constant diagonal,
...
The layout also carries known invariants and ports when tensor shapes alone cannot: coordinates excluded from $M$ or the base potential, periodic position coordinates represented internally by sine/cosine features, sparse actuator-to-DOF topology, near-zero damping priors, and known unilateral joint limits. These restrictions affect only learned mechanical objects; the state remains raw $(q,\dot q)$, so the exact kinematic block $\dot q$ is unchanged. Cartpole uses all of them: its mass and gravity potential cannot depend on cart translation, its hinge mechanics are periodic, its actuator drives only the slider, and its rail reaction has a separate passive spring/damping port.
...
with the correct physics built in: a vertical foot force has zero generalized component along horizontal translation (it cannot propel), while the tangential row carries the unit root-$x$ entry — propulsion comes from friction. The magnitudes are a unilateral Hunt–Crossley normal force and regularized Coulomb friction,

$$
\lambda_i = \varphi(g_i)\,\big(k_i + c_i\,\mathrm{relu}(-\dot g_i)\big),\qquad
\varphi(g) = w\,\mathrm{softplus}(-g/w),\qquad
f_{t,i} = -\mu_i\,\lambda_i\,\tanh(v_{t,i}/v_s),
$$

with $\dot g_i = J_{n,i}\cdot\dot q$, $v_{t,i} = J_{t,i}\cdot\dot q$, per-contact stiffness $k_i$, compression damping $c_i$, and friction coefficient $\mu_i$ all positive via softplus. The momentum balance of §2.3 gains
...
**Guarded warmup and target dynamics.** The optimizer trains a live dynamics model, but the critic reads a separate frozen target copy. After each fit, the live model is checked on an independent replay minibatch with the same finite-duration flow used by training and quadrature. A candidate is published only when its parameters, recorded-duration endpoints, nominal target flow, mass diagnostics, and relative flow error are finite and within threshold. The first accepted publication is a hard copy and later ones enter by EMA; rejected finite candidates continue learning without touching the critic, while a non-finite candidate is restored from the last accepted copy and its optimizer state is cleared. Learned quadrature remains on the model-free finite-difference target until both the required number of accepted publications and the scalar value-head warmup are complete. After takeover there is no silent model-free fallback: a rejected live update leaves the last accepted learned target in service. A non-trainable oracle needs no publication gate.
...
flowchart TD
    ...
    ST --> CFP["contact port: J_n lam + J_t f_t from gap nets g_i, h_i"]
...
For the irregular cartpole run the endpoint terms are duration-balanced with
$w_i=(h_{\max}/\max(\Delta t_i,h_{\max}))^2$ and normalized by $\sum_i w_i$. Since an endpoint error from a fixed local drift error grows approximately as $\Delta t_i$, this removes its quadratic duration leverage without shortening or discarding any transition. The multi-transition objective uses the same weight at every valid outer endpoint. Its configured $H=4$ rollout is introduced as a curriculum: the first 5,000 dynamics fits use $H=1$; afterward local $H=1$ fits remain the common update and every fourth fit uses the full $H=4$ window. Cartpole performs a dynamics fit every second critic update. This keeps both local and open-loop supervision without rebuilding a worst-case 60-step higher-order graph on every environment step.
...
The matrix-valued $M(q)$ keeps a forward-mode Jacobian, while the scalar potential gradient is evaluated with the exact reverse-mode gradient transform. This changes only the derivative implementation—not the learned function or objective—and avoids taking one forward-mode tangent pass per position coordinate for a scalar output.
...
**Stability guards.** Internal integration and later outer transitions evaluate the drift at model-predicted states, where the velocity-quadratic Coriolis term can overflow. A data-scaled cumulative-displacement bound therefore guards intermediate internal states and off-manifold rollout endpoints. The first endpoint from a real buffer state remains unclamped in the loss, preserving both a corrective gradient and exact $H{=}1$ equivalence; a bounded copy seeds a longer rollout. Every dynamics fit clips gradients to a global norm of 10 and immediately checks the loss, gradient norm, and post-step parameters. A skipped or non-finite update is rolled back before it can be published. The expensive held-out recorded/nominal flow audit runs every tenth fit; between audits the critic continues to read the last frozen accepted target. The healthy target path reduces all component-finiteness flags once. If it fails, a diagnostic pass identifies the bad value component and reruns integration with per-step drift/state checks, retaining actionable failures without synchronizing the accelerator at every internal step.
...
### C.2 Diagonal damping $D = \mathrm{diag}\big(\mathrm{softplus}(\log d)\big)$

**softplus.** $\mathrm{softplus}(z) = \log(1 + e^{z})$ maps any real number to a strictly positive one — for large $z$ it is $\approx z$, for large negative $z$ it is $\approx e^{z}$ (small but positive). It is a smooth version of $\max(0,z)$, so a parameter or a network output can range over all reals while the quantity it feeds stays positive. That is how each damping coefficient — and every positive contact-port parameter in C.3 — is held nonnegative.
...
The port of §2.7 must meet three structural constraints, and like C.1/C.2 each holds for *any* network output, so training stays unconstrained.

**The $J^\top$ structure is virtual work, not a choice.** For a point contact, a force $f$ applied at a material point $p(q)$ produces the generalized force $Q = (\partial p/\partial q)^\top f = J^\top f$. The port's contact point has height $g_i(q)$ and horizontal position $x_{\text{root}} + h_i(q)$, so the vertical force row is $\partial g_i/\partial q$ and the horizontal one is $e_x + \partial h_i/\partial q$ — the force directions are *tied by autodiff to the same functions whose rates give the contact velocities* $\dot g_i, v_{t,i}$. This coupling is what the energy-balance test checks: the port's power must equal $\sum_i (\lambda_i\dot g_i + f_{t,i} v_{t,i})$ along the drift.

**Unilateral.** $\lambda_i = \varphi(g_i)(k_i + c_i\,\mathrm{relu}(-\dot g_i))$ is a product of nonnegative factors — $\varphi \ge 0$ (softplus), $k_i, c_i > 0$ (softplus of raw parameters), $\mathrm{relu} \ge 0$ — so $\lambda_i \ge 0$: the ground pushes, never pulls. $\varphi(g) = w\,\mathrm{softplus}(-g/w)$ is a smoothed $\max(0,-g)$ (penetration depth); for positive gaps it decays like $w e^{-g/w}$, so an out-of-contact point transmits (numerically) nothing. The width $w$ is a fixed constant: since $g_i$ is a learned function, any rescaling of $w$ can be absorbed into $g_i$, so $w$ only sets the sharpness of force onset relative to the learned gap scale.

**Passive.** Three parts, three arguments. The spring part $k_i\,\varphi(g_i)\,\nabla g_i$ is the exact gradient of the penalty potential $k_i\,\Phi(g_i)$ with $\Phi' = \varphi$ — a conservative storage term, like $\nabla V$. The compression part transmits power $c_i\,\varphi\,\mathrm{relu}(-\dot g_i)\,\dot g_i \le 0$, since it is nonzero only where $\dot g_i < 0$. The friction power is $f_{t,i}\,v_{t,i} = -\mu_i \lambda_i\, v_{t,i}\tanh(v_{t,i}/v_s) \le 0$, since $z\tanh(z) \ge 0$. So the port adds storage and dissipation, never free energy, and the passivity story of §2.3 survives with the gap potentials counted in the storage.

**Friction is regularized Coulomb.** $f_t = -\mu\lambda\tanh(v_t/v_s)$ saturates at $\pm\mu\lambda$ (the Coulomb cone) and passes smoothly through $v_t = 0$ instead of the discontinuous $\mathrm{sign}$ — the standard smoothing for differentiable simulation; $v_s$ sets the stick–slip velocity scale.
...
| $D$ | constant diagonal PSD damping on momentum (passive joint damping) |
