...
| `benchmarks/run_ct_rl.py` | `dynamics_source` value `structured` and `dynamics_contact_force` (number of learned contact points). |
...
**Why the quadrature form is stable (contraction).** The paper's convergence proof (Theorem 4.1) rests on the short-horizon dynamic-programming operator $\Phi_\tau^{(\alpha)}$ (paper Eq. 55), which reads the value only as $\mathbb E[e^{-\beta\tau}V(X_\tau)]$ — an expectation of point evaluations, scaled by the discount. Lemma D.1 shows this makes it a strict sup-norm contraction,

$$
\big\lVert \Phi_\tau^{(\alpha)}(V) - \Phi_\tau^{(\alpha)}(W) \big\rVert_\infty \;\le\; e^{-\beta\tau}\,\lVert V - W\rVert_\infty ,
\tag{paper Eq. 56}
$$

and the proof is two lines because evaluating a function at a point never expands a sup-norm difference (gain $1$ at every frequency) and $e^{-\beta\tau}<1$ supplies the strict modulus; Banach then bounds the iterates from any start. The Picard–Hamiltonian operator $T_\tau^{(\alpha)}V = V + \tau H^{(\alpha)}(V)$ (paper Eq. 59) instead reads the value through its generator $\mathcal L^a V = b\cdot\nabla V$, an unbounded operator on $(C, \lVert\cdot\rVert_\infty)$: an error mode $\varepsilon\sin(k\cdot x)$ in $V$ has sup-norm $\varepsilon$ but contributes $\tau\,\lvert b\cdot k\rvert\,\varepsilon$ to the target, growing without bound in the wavenumber $k$. The paper states $T^{(\alpha)}$ is not a contraction on $(C,\lVert\cdot\rVert_\infty)$ and controls it only as a perturbation of $\Phi_\tau^{(\alpha)}$ under smoothness assumptions.

The two model-based targets sit on the two sides of this dichotomy. The first-order target is the $T$-form: $V^{\text{tgt}}$ enters through $\nabla V^{\text{tgt}}$, and the training loop — critic regresses onto $y$, value head regresses onto the critic (§B.3), Polyak copies into $V^{\text{tgt}}$ (§B.6), $V^{\text{tgt}}$ re-enters $y$ — has per-mode gain $\approx\lvert 1-\beta + i\,\Delta t_{\text{default}}\,b\cdot k\rvert$, which exceeds $1$ for rough components. Regression noise in a neural value head is exactly high-frequency, so the loop amplifies it geometrically: that is the blow-up channel. The quadrature target is the $\Phi$-form with $\sigma=0$: the expectation degenerates to composition with the model flow, $V^{\text{tgt}}(\hat x_m)$, and composition is sup-norm non-expansive — a value error of sup-norm $\varepsilon$ perturbs the target by at most $(1+\beta)\varepsilon$ no matter how rough the error is, and by $\approx(1-\beta)\varepsilon$ on the smooth components, the first-order expansion of the paper's modulus $e^{-\beta\tau}$. The "uses no autograd" property above is this in code form: no derivative of $V^{\text{tgt}}$ appears anywhere in the target.

One caveat: keeping the discount as the $-\beta V^{\text{tgt}}(x)$ lump rather than the exact factor $e^{-\beta}V^{\text{tgt}}(\hat x_m)$ trades the strict modulus for parity with the first-order convention — the two differ by $\beta\,(\Delta V(\hat x_m)-\Delta V(x))$ on a value error $\Delta V$, so the worst-case gain is $1+\beta$ rather than $e^{-\beta}<1$. The essential stability property survives: the loop gain is bounded independent of the roughness of $V^{\text{tgt}}$, where the generator form's gain is unbounded. The remaining blow-up channel is the orbit itself — $b_\phi$ evaluated at its own predictions — which the §B.2 guards handle separately.
...
| `mbq_vhead_quad_buf1m` | ceiling isolation: oracle (`mujoco`) drift + sub-step quadrature + V-head + 1M buffer — the same target construction as the cforce family with model error removed entirely. The cforce 2×2 peaked at 295–357 under every fit-mode/buffer combination while `top` reaches ~800; if this mode also caps near 300 the backup construction is the ceiling, and if it climbs toward `mbq_vhead`'s 600–950 the residual gap is the learned model's coverage where the target reads it. |
